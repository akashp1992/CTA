...
export default function CustomPaginationActionsTable(props) {
    ...
    React.useEffect(() => {
        const request = {
            onSuccess: (res) => {
                setTeamData(res.data.sort((a, b) => a.id - b.id))
                const data = []
                res.data.map((items) => {
                    items.team_members.map((item) => {
                        data.push(
                            {
                    ...
                    // data.push(items.team_members)
                    setTeamMemberData(data.filter((el) => el === el))
                })
            },
            onFail: (err) => {
        ...
        const requestTeamMember = {
            onSuccess: (res) => {
                ...
                res.data.map((items) => {
                    ...
                    setListOfItems(data1.filter((el) => el !== undefined))
                    const filterId = listOfItems.filter((el) => el.id === items.id)
                })
    ...
    const filter = teamMemberData.map((items) => {
        return listOfItems.find(el => {
            return el.id === items.team_id
        })
